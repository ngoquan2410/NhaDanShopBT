            ...
            <div className="space-y-3">
              ...
              {product && product.variants.length > 0 && (
                  <>
                    ...
                    {/* Mobile cards: full action set (set default / edit / delete) reachable without horizontal scroll */}
                        ...
                        return (
                            <div
                                ...
                                className={cn(
                                    ...
                                    stockStatus === "out-of-stock" && "bg-danger-soft/30 border-danger/30",
                                    stockStatus === "low-stock" && "bg-warning-soft/30 border-warning/30",
                                )}
                            >
                              <div className="flex items-start justify-between gap-2">
                                <div className="min-w-0">
                                  <p className="font-mono text-[11px] text-muted-foreground">{v.code}</p>
                          ...
                          {v.isSellable === false ? "NVL" : "Bán lẻ"}
                                ...
                                <div className="flex items-center gap-0.5 shrink-0">
                                  <button onClick={() => handleSetDefault(v)} title={v.isDefault ? "Mặc định" : "Đặt làm mặc định"} className={cn("p-1.5 rounded hover:bg-muted", v.isDefault ? "text-accent" : "text-muted-foreground/40")}>
                                    <Star className={cn("h-4 w-4", v.isDefault && "fill-accent")} />
                                  </button>
                                  <button onClick={() => openEditVariant(v)} title="Sửa" className="p-1.5 rounded hover:bg-muted text-muted-foreground hover:text-foreground"><Pencil className="h-4 w-4" /></button>
                                  <button onClick={() => setConfirmDeleteVariant(v)} title="Xóa" className="p-1.5 rounded hover:bg-muted text-muted-foreground hover:text-danger"><Trash2 className="h-4 w-4" /></button>
                                </div>
                              </div>
                              <div className="grid grid-cols-2 gap-2 text-xs">
                                <div>
                                  <p className="text-muted-foreground">ĐV bán / nhập</p>
                                  <p className="font-medium">{v.sellUnit} / {v.importUnit} ({v.piecesPerImportUnit})</p>
                                </div>
                                <div>
                                  <p className="text-muted-foreground">Giá bán / nhập</p>
                                  <p className="font-medium">{formatVND(v.sellPrice)} <span className="text-muted-foreground">/ {formatVND(v.costPrice)}</span></p>
                                </div>
                                <div>
                                  <p className="text-muted-foreground">Tồn / Min</p>
                                  <p className="font-medium flex items-center gap-1">
                                    <span>{v.stock} / {v.minStock}</span>
                                    <StatusBadge status={stockStatus} />
                                  </p>
                                </div>
                                <div>
                                  <p className="text-muted-foreground">HSD</p>
                                  <p className="font-medium">{v.expiryDays > 0 ? `${v.expiryDays} ngày` : "—"}</p>
                                </div>
                              </div>
                            </div>
                        );
