        ...
        {!loading && filtered.length === 0 ? (
            ...
            <>
              ...
              <div className="sm:hidden space-y-2">
                {tc.pageRows.map(cat => (
                    <div key={`m-${cat.id}`} className="bg-card rounded-lg border p-3">
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          <p className="font-medium text-sm truncate">{cat.name}</p>
                          {cat.description && <p className="text-[11px] text-muted-foreground line-clamp-2 mt-0.5">{cat.description}</p>}
                          <div className="flex items-center gap-2 mt-1.5">
                            <StatusBadge status={cat.active ? "active" : "inactive"} />
                            <span className="text-[11px] text-muted-foreground">{cat.productCount} sản phẩm</span>
                          </div>
                        </div>
                        <div className="flex items-center gap-0.5 shrink-0">
                          <button onClick={() => handleToggle(cat)} title={cat.active ? "Ngưng" : "Kích hoạt"} className="p-1.5 text-muted-foreground hover:text-foreground rounded hover:bg-muted"><Power className="h-3.5 w-3.5" /></button>
                          <button onClick={() => openEdit(cat)} title="Sửa" className="p-1.5 text-muted-foreground hover:text-foreground rounded hover:bg-muted"><Pencil className="h-3.5 w-3.5" /></button>
                          <button onClick={() => setConfirmDelete(cat)} title="Xóa" className="p-1.5 text-muted-foreground hover:text-danger rounded hover:bg-muted"><Trash2 className="h-3.5 w-3.5" /></button>
                        </div>
                      </div>
                    </div>
                ))}
