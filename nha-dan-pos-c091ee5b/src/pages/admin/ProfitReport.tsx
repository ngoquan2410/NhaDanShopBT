  ...
  return (
      <div className="space-y-4 admin-dense">
        ...
        <div className="bg-card rounded-lg border overflow-hidden">
          ...
          {isFiltered ? (
              <>
                {/* Mobile cards (filtered detail rows) */}
                <ul className="sm:hidden divide-y">
                  {filteredDetailRows.map((r) => (
                      <li key={`m-${r.id}`} className="px-3 py-2.5">
                        <div className="flex items-start justify-between gap-2">
                          <span className="font-medium text-sm min-w-0 flex-1 truncate">{r.name}</span>
                          <span className="text-success font-semibold text-sm shrink-0">{money(r.profit)}</span>
                        </div>
                        <div className="grid grid-cols-4 gap-1 mt-1.5 text-[11px]">
                          <div><span className="block text-[10px] text-muted-foreground">SL</span>{r.qty != null ? formatNumber(r.qty) : "—"}</div>
                          <div><span className="block text-[10px] text-muted-foreground">DT</span>{money(r.revenue)}</div>
                          <div><span className="block text-[10px] text-muted-foreground">Vốn</span>{money(r.cost)}</div>
                          <div className="text-right"><span className="block text-[10px] text-muted-foreground">Biên</span>{pct(r.margin)}</div>
                        </div>
                      </li>
                  ))}
                  <li className="px-3 py-2.5 bg-muted/50 font-semibold">
                    <div className="flex items-center justify-between text-sm"><span>Tổng (hàng có dữ liệu)</span><span className="text-success">{formatVND(footerFiltered.prof)}</span></div>
                    <div className="grid grid-cols-4 gap-1 mt-1.5 text-[11px]">
                      <div><span className="block text-[10px] text-muted-foreground">SL</span>{formatNumber(footerFiltered.q)}</div>
                      <div><span className="block text-[10px] text-muted-foreground">DT</span>{formatVND(footerFiltered.rev)}</div>
                      <div><span className="block text-[10px] text-muted-foreground">Vốn</span>{formatVND(footerFiltered.cst)}</div>
                      <div className="text-right"><span className="block text-[10px] text-muted-foreground">Biên</span>{formatPercent(footerFiltered.margin)}</div>
                    </div>
                  </li>
                </ul>
              ...
              <>
                {/* Mobile cards (period summary) */}
                <ul className="sm:hidden divide-y">
                  {(reportData?.profitRows ?? []).map((row, i) => (
                      <li key={`mp-${i}`} className="px-3 py-2.5">
                        <div className="flex items-center justify-between gap-2">
                          <span className="font-medium text-sm">{row.period}</span>
                          <span className="text-success font-semibold text-sm">{formatVND(row.profit)}</span>
                        </div>
                        <div className="grid grid-cols-4 gap-1 mt-1.5 text-[11px]">
                          <div><span className="block text-[10px] text-muted-foreground">DT</span>{formatVND(row.revenue)}</div>
                          <div><span className="block text-[10px] text-muted-foreground">Vốn</span>{formatVND(row.cost)}</div>
                          <div><span className="block text-[10px] text-muted-foreground">Biên</span>{formatPercent((row.margin ?? 0) / 100)}</div>
                          <div className="text-right"><span className="block text-[10px] text-muted-foreground">HĐ</span>{row.invoiceCount}</div>
                        </div>
                      </li>
                  ))}
                </ul>
