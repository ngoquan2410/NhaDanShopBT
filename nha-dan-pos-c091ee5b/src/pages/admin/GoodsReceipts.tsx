                        ...
                        <div key={r.id} className="bg-card rounded-lg border p-3" onClick={() => setDetail(r)}>
                          <div className="flex items-start justify-between gap-2 mb-2">
                            ...
                            <div className="flex items-center gap-1 shrink-0">
                              ...
                              <RowActions
                                  actions={[
                                    { label: "Xem chi tiết", icon: <Eye className="h-3.5 w-3.5" />, onClick: () => setDetail(r) },
                                    { label: "In phiếu", icon: <Printer className="h-3.5 w-3.5" />, onClick: () => setDetail(r) },
                                    {
                                      separatorBefore: true,
                                      label: "Xóa phiếu",
                                      icon: <Trash2 className="h-3.5 w-3.5" />,
                                      danger: true,
                                      disabled: ui !== "CONFIRMED_DELETE_ALLOWED",
                                      disabledReason: ui === "CONFIRMED_DOWNSTREAM_BLOCKED" ? "Hàng từ phiếu này đã được bán" : ui === "VOIDED" ? "Phiếu đã void" : "Không thể xóa",
                                      onClick: () => {
                                        if (ui === "CONFIRMED_DOWNSTREAM_BLOCKED") setDeleteBlocked(r);
                                        else setDeleteTarget(r.id);
                                      },
                                    },
                                  ]}
                              />
